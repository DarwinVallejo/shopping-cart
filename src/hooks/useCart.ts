...
const useCart = (): UseCart => {
  ...
  const addToCart = (newProduct: product): number => {
    const cartProducts = [...cartState];
    const cartProductIndex = cartProducts.findIndex((product) => product.id === newProduct.id);
  ...
  const removeFromCart = (id: number): number => {
    ...
    const cartProductIndex = cartProducts.findIndex(product => product.id === id);
